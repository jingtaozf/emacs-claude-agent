...
Returns a process state object."
  (let* ((cli-path (expand-file-name
                    (or (plist-get options :cli-path)
    ...
    ;; Build command
    (let* ((command (cons cli-path args))
      ...
      (when session-key
        ...
        (claude-agent--verbose-insert-query-header session-key prompt cli-path args default-directory env-vars))
...
Claude Agent SDK integrates with Emacs's native hook system to intercept and
control Claude's tool use via the bidirectional control protocol. This allows
you to use familiar =add-hook= and =remove-hook= functions to customize Claude's
behavior.

** Hook Variables

Define Emacs-style hooks for each Claude control protocol event.

#+BEGIN_SRC elisp
(defvar claude-agent-pre-tool-use-hook nil
  "Hook run before Claude executes a tool via control protocol.

Functions are called with three arguments:
  INPUT-DATA: plist with :tool_name and :tool_input
  TOOL-USE-ID: string ID of the tool use (may be nil)
  CONTEXT: plist with :signal for cancellation

Each function should return nil to allow the tool, or a plist with
hook output to modify or deny the tool execution.

Hook output plist format:
  :reason                  - String explaining the decision
  :systemMessage           - String shown to Claude in conversation
  :hookSpecificOutput      - Plist with event-specific fields:
    :hookEventName         - \"PreToolUse\"
    :permissionDecision    - \"allow\" or \"deny\"
    :permissionDecisionReason - String explaining permission decision
    :updatedInput          - Modified tool input (if allowing with changes)

Example - Block dangerous bash commands:
  (defun my-block-rm-rf (input-data tool-use-id context)
    (let ((tool-name (plist-get input-data :tool_name))
          (tool-input (plist-get input-data :tool_input)))
      (when (and (equal tool-name \"Bash\")
                 (string-match-p \"rm -rf\"
                               (plist-get tool-input :command)))
        (list :reason \"Dangerous command blocked\"
              :systemMessage \"🚫 'rm -rf' commands are not allowed\"
              :hookSpecificOutput
  ...
  (add-hook 'claude-agent-pre-tool-use-hook #'my-block-rm-rf)

This hook is made buffer-local automatically when used in minor modes.")

(defvar claude-agent-post-tool-use-hook nil
  "Hook run after Claude executes a tool via control protocol.

Functions are called with the same arguments as pre-tool-use-hook, but
INPUT-DATA also contains :tool_response with the tool's output.

Hook output plist format:
  :reason                  - String explaining feedback
  :systemMessage           - String shown to Claude
  :continue_               - Boolean (if false, stops execution)
  :stopReason              - String explaining why execution stopped
  :hookSpecificOutput      - Plist with:
    :hookEventName         - \"PostToolUse\"
    :additionalContext     - String with extra context for Claude

Example - Stop on critical errors:
  (defun my-stop-on-critical-error (input-data tool-use-id context)
  ...
  (add-hook 'claude-agent-post-tool-use-hook
            #'my-stop-on-critical-error)

This hook is made buffer-local automatically when used in minor modes.")

(defvar claude-agent-user-prompt-submit-hook nil
  "Hook run when user submits a prompt to Claude.

Functions are called with three arguments (tool-use-id will be nil).

Hook output plist format:
  :hookSpecificOutput      - Plist with:
    :hookEventName         - \"UserPromptSubmit\"
    :additionalContext     - String with context to inject
  ...
  (defun my-add-project-context (input-data tool-use-id context)
    (list :hookSpecificOutput
          (list :hookEventName \"UserPromptSubmit\"
                :additionalContext
                  \"This project uses Python 3.11 and pytest.\")))

  (add-hook 'claude-agent-user-prompt-submit-hook
            #'my-add-project-context)

This hook is made buffer-local automatically when used in minor modes.")

(defvar claude-agent-session-start-hook nil
  "Hook run when a Claude session starts.
...
Hook output plist format:
  :hookSpecificOutput      - Plist with:
    ...
    :additionalContext     - String with session-wide context
...
Track control protocol request/response state in process state.

#+BEGIN_SRC elisp
(defun claude-agent--process-state-add-fields (state)
  "Add control protocol fields to process STATE if not present.
This extends the cl-defstruct without modifying the definition."
  ;; Control protocol state (stored as plist in process-get)
  (unless (process-get (claude-agent--process-state-process state)
                       'claude-control-requests)
    (process-put (claude-agent--process-state-process state)
                 'claude-control-requests
                 (make-hash-table :test 'equal)))
  (unless (process-get (claude-agent--process-state-process state)
                       'claude-request-counter)
    (process-put (claude-agent--process-state-process state)
                 'claude-request-counter
                 0)))
#+END_SRC
...
Run Emacs hooks and merge results for control protocol responses.

#+BEGIN_SRC elisp
(defun claude-agent--run-hooks-for-event (hook-symbol input-data tool-use-id context)
  "Run Emacs hooks in HOOK-SYMBOL and merge results.

HOOK-SYMBOL: Symbol like 'claude-agent-pre-tool-use-hook
INPUT-DATA: plist with :tool_name, :tool_input, etc.
TOOL-USE-ID: string ID or nil
CONTEXT: plist with :signal

Returns: Merged hook output plist, or nil if all hooks allow."
  (let ((results '())
        (denied nil)
        (hooks (symbol-value hook-symbol)))
    ;; Run all hook functions
    (dolist (hook-fn hooks)
      (when (functionp hook-fn)
        (condition-case err
            (let ((result (funcall hook-fn input-data tool-use-id context)))
              (when result
                (push result results)
                ;; Check for deny decision
                (when (equal "deny"
                            (plist-get
                              (plist-get result :hookSpecificOutput)
                              :permissionDecision))
                  (setq denied t)
                  ;; Stop at first deny
                  (return))))
          (error
           (claude-agent--debug-log "Hook error: %s"
                                    (error-message-string err))))))

    ;; Merge results
    (cond
     ;; If any hook denied, return first deny
     (denied
      (cl-find-if (lambda (r)
                    (equal "deny"
                           (plist-get
                             (plist-get r :hookSpecificOutput)
                             :permissionDecision)))
                  results))
     ...
     (results
      (let ((merged '()))
        ...
        merged))
     ;; No results - allow by default
     (t nil))))

(defun claude-agent--hook-event-name-to-symbol (event-name)
  "Convert EVENT-NAME string to hook symbol.
PreToolUse -> claude-agent-pre-tool-use-hook
PostToolUse -> claude-agent-post-tool-use-hook"
  ;; Convert CamelCase to kebab-case
  ;; 1. Insert dash before every uppercase: PreToolUse -> -Pre-Tool-Use
  ;; 2. Remove leading dash: -Pre-Tool-Use -> Pre-Tool-Use
  ;; 3. Downcase: Pre-Tool-Use -> pre-tool-use
  (let* ((case-fold-search nil)  ; Disable case-insensitive matching
         (with-dashes (replace-regexp-in-string
                       "\\([A-Z]\\)" "-\\1"
                       event-name))
         (without-leading-dash (if (string-prefix-p "-" with-dashes)
                                   (substring with-dashes 1)
                                 with-dashes))
         (kebab-case (downcase without-leading-dash)))
    (intern (concat "claude-agent-" kebab-case "-hook"))))
#+END_SRC
    ...
    (condition-case err
        (cond
         ...
         ((equal subtype "hook_callback")
          (let* ((callback-id (plist-get request-data :callback_id))
                 (input-data (plist-get request-data :input))
                 (tool-use-id (plist-get request-data :tool_use_id))
                 (context (list :signal nil))
                 ;; Extract hook event name from input
                 (hook-event (plist-get input-data :hook_event_name))
                 ;; Convert to hook symbol
                 (hook-symbol (claude-agent--hook-event-name-to-symbol hook-event))
                 ;; Run hooks
                 (hook-output (claude-agent--run-hooks-for-event
                              hook-symbol input-data tool-use-id context)))
            ;; Send response
            (claude-agent--send-control-response
             process request-id "success"
             (or hook-output (list)))))

         ;; Tool permission (uses PreToolUse hooks)
         ((equal subtype "can_use_tool")
          (let* ((tool-name (plist-get request-data :tool_name))
                 (tool-input (plist-get request-data :input))
                 (input-data (list :tool_name tool-name
                                  :tool_input tool-input))
                 (context (list :signal nil))
                 ;; Run PreToolUse hooks
                 (hook-output (claude-agent--run-hooks-for-event
                              'claude-agent-pre-tool-use-hook
                              input-data nil context)))
            ;; Convert hook output to permission response
            (if hook-output
                (let* ((hook-specific (plist-get hook-output :hookSpecificOutput))
                       (decision (plist-get hook-specific :permissionDecision))
                       (updated-input (plist-get hook-specific :updatedInput)))
                  (if (equal decision "deny")
                      ;; Deny with message
                      (claude-agent--send-control-response
                       process request-id "success"
                       (list :behavior "deny"
                             :message (or (plist-get hook-output :systemMessage)
                                         (plist-get hook-output :reason)
                                         "Tool use denied by hook")))
                    ;; Allow (possibly with modified input)
                    (claude-agent--send-control-response
                     process request-id "success"
                     (list :behavior "allow"
                           :updatedInput (or updated-input tool-input)))))
              ;; No hooks - allow by default
              (claude-agent--send-control-response
               process request-id "success"
               (list :behavior "allow"
...
(ert-deftest claude-agent-test-context-too-long-detection ()
  "Test context/prompt too long detection."
  ...
  (should (claude-agent--context-too-long-p "context too long"))
  (should (claude-agent--context-too-long-p "exceeds context limit"))
  (should (claude-agent--context-too-long-p "token limit reached"))
  (should-not (claude-agent--context-too-long-p "some other error")))

(ert-deftest claude-agent-test-session-needs-reset ()
  "Test combined session reset detection."
  (should (claude-agent--session-needs-reset-p "session expired"))
  (should (claude-agent--session-needs-reset-p "Prompt is too long"))
  (should-not (claude-agent--session-needs-reset-p "network error")))
...
ORG-OUTPUT-LEVEL is the target org section level for output headers.
CWD is the working directory (project root) where operations should be performed."
  (let ((parts nil))
    ...
    ;; Org output level hint
    (when (and org-output-level (> org-output-level 0))
      (push (format "When creating org section headers in output, use level %d (%s) or deeper"
                    org-output-level
                    (make-string org-output-level ?*))
            parts))
