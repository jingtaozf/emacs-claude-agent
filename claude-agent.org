...
If STREAMING-P is non-nil, use streaming input mode."
  (let ((args (list "--output-format" "stream-json" "--verbose")))
    ...
    (when (and streaming-p claude-agent-permission-callback)
  ...
  (let* ((request-id (plist-get request :request_id))
    ...
    (condition-case err
        (cond
         ...
         ;; Tool permission (uses PreToolUse hooks, then callback)
         ((equal subtype "can_use_tool")
          (let* ((tool-name (plist-get request-data :tool_name))
            ...
            (if hook-output
              ...
              ;; No hooks decided - use permission callback
              (if claude-agent-permission-callback
                  (let* ((session-id (process-get process :session-id))
                         ...
                         (result (funcall claude-agent-permission-callback
                                         tool-name tool-input context)))
                    (claude-agent--send-control-response
                     process request-id "success"
                     (list :behavior (plist-get result :behavior)
...
the callback function is invoked to decide. Built-in implementations:
- =claude-agent-permission-prompt= - Interactive y-or-n-p (default)
- =claude-agent-permission-auto-allow= - Auto-approve everything

#+BEGIN_SRC elisp
(defcustom claude-agent-permission-callback #'claude-agent-permission-prompt
  "Function called when Claude requests tool permission.
Receives: TOOL-NAME, TOOL-INPUT, CONTEXT.
Returns: plist (:behavior \"allow\") or (:behavior \"deny\" :message \"...\").
