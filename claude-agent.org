...
(defun claude-agent-usage-format-reset-time (iso-time)
  ...
  (if iso-time
      (let* ((time (parse-iso8601-time-string iso-time))
             ...
             (hours (/ diff 3600))
             (mins (/ (mod diff 3600) 60)))
        (if (> diff 0)
            (format "%.0fh%.0fm" hours mins)
          "soon"))
    "N/A"))
#+END_SRC
...
(defun claude-agent--verbose-insert-query-header (session-key prompt)
...
(defun claude-agent--verbose-format-message (msg)
  ...
  (cond
   ;; Tool use block
   ((claude-agent-tool-use-block-p msg)
    (format "▶ %s\n"
            (propertize (claude-agent-tool-use-block-name msg)
                        'face 'font-lock-function-name-face)))

   ;; Tool result block
   ((claude-agent-tool-result-block-p msg)
    (if (claude-agent-tool-result-block-is-error msg)
        (format "◀ %s\n" (propertize "ERROR" 'face 'error))
      ""))  ; Don't show success results (too verbose)

   ;; Thinking block
   ((claude-agent-thinking-block-p msg)
    (format "%s\n" (propertize "thinking..." 'face 'font-lock-comment-face)))

   ;; Text block
   ((claude-agent-text-block-p msg)
    (claude-agent-text-block-text msg))

   ;; System message
   ((claude-agent-system-message-p msg)
    (let ((subtype (claude-agent-system-message-subtype msg)))
      (when (member subtype '("init" "error"))
        (format "ℹ %s\n" subtype))))
