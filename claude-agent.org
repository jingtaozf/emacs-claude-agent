...
Returns: Merged hook output plist, or nil if all hooks allow."
  (let ((results '())
    ...
    ;; Run all hook functions, stop at first deny
    (catch 'hook-denied
...
MCPToolUse -> claude-agent-mcp-tool-use-hook"
  ;; Convert CamelCase to kebab-case (handles acronyms correctly)
  ;; 1. Insert dash before uppercase that follows lowercase: PreToolUse -> Pre-ToolUse
  ;; 2. Insert dash before uppercase that precedes lowercase (acronyms): MCPToolUse -> MCP-ToolUse
  ;; 3. Downcase: Pre-Tool-Use -> pre-tool-use
  (let* ((case-fold-search nil)  ; Disable case-insensitive matching
         ;; Step 1: lowercase followed by uppercase
         (step1 (replace-regexp-in-string
                 "\\([a-z]\\)\\([A-Z]\\)" "\\1-\\2"
                 event-name))
         ;; Step 2: uppercase followed by uppercase+lowercase (for acronyms)
         (step2 (replace-regexp-in-string
                 "\\([A-Z]+\\)\\([A-Z][a-z]\\)" "\\1-\\2"
                 step1))
         (kebab-case (downcase step2)))
...
Example:
  ...
  (let* ((formatted-messages (claude-agent--format-messages-for-title messages))
         (prompt (format "Please write a %d character or less title for the following conversation. \
Output ONLY the title, nothing else - no quotes, no explanation, no punctuation at the end.

%s"
                         claude-agent-title-max-chars
                         formatted-messages))
         (system-prompt (format "You are a conversation title generator. \
Generate concise, descriptive titles that capture the main topic. \
Keep titles under %d characters. Output only the title text."
                                claude-agent-title-max-chars))
         (options (claude-agent-options
