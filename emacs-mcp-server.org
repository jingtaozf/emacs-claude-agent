...
Tool definitions follow the MCP schema format, mapping tool names to handlers
and JSON Schema input specifications. These are registered with the MCP server
when =claude-mcp-org-enable= is called.

*** Section Tool Definitions

Section tools provide CRUD operations on org headings and their content.
These form the primary interface for navigating and editing org documents.

#+NAME: org-tool-defs-section
#+BEGIN_SRC elisp
(defvar claude-mcp-org--section-tool-definitions
  (list
   ...
   `((name . "org_delete_section")
     ...
     (inputSchema . ((type . "object")
                     ...
                     (required . ["file_path" "section_path"])))))
  "Section manipulation tool definitions.")
#+END_SRC

*** Property Tool Definitions

Property tools manage org property drawers (=:PROPERTIES:= blocks).
Properties store metadata like timestamps, IDs, and custom key-value pairs.

#+NAME: org-tool-defs-property
#+BEGIN_SRC elisp
(defvar claude-mcp-org--property-tool-definitions
  (list
     ...
     (inputSchema . ((type . "object")
                     (properties . ((file_path . ((type . "string")
                                    ...
                                    (section_path . ((type . "string")
                                                     (description . "Section path")))))
                     (required . ["file_path" "section_path"])))))
  "Property manipulation tool definitions.")
#+END_SRC

*** Tag Tool Definitions

Tag tools manage org heading tags (the =:tag1:tag2:= syntax at end of headings).
Tags enable filtering, searching, and categorizing org entries.

#+NAME: org-tool-defs-tag
#+BEGIN_SRC elisp
(defvar claude-mcp-org--tag-tool-definitions
  (list
   `((name . "org_get_tags")
   ...
   `((name . "org_remove_tag")
     ...
     (inputSchema . ((type . "object")
                     ...
                     (required . ["file_path" "section_path" "tag"])))))
  "Tag manipulation tool definitions.")
#+END_SRC

*** Combined Tool Definitions

Aggregate all tool definitions into a single list for registration.

#+NAME: org-tool-defs-combined
#+BEGIN_SRC elisp
(defvar claude-mcp-org--tool-definitions
  (append claude-mcp-org--section-tool-definitions
          claude-mcp-org--property-tool-definitions
          claude-mcp-org--tag-tool-definitions)
  "List of all MCP tool definitions for org-mode operations.")
#+END_SRC
