...
If you prefer a simpler terminal-based approach, consider [[https://github.com/anthropics/claude-code/tree/main/packages/agent-shell][agent-shell]] - an official Anthropic package that provides a shell-mode interface for Claude Code in Emacs.

| Approach | Best For |
|----------+----------|
| *claude-agent* | Literate programming, org-mode notebooks, session management, MCP integration |
| *agent-shell* | Quick terminal-style interactions, simpler setup |
