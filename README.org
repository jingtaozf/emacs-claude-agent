...
- [[#features][Features]]
  ...
  - [[#permission-functions][Permission Functions]]
    - [[#quick-example---block-dangerous-commands][Quick Example - Block Dangerous Commands]]
    - [[#available-hooks][Available Hooks]]
    - [[#buffer-local-hooks][Buffer-Local Hooks]]
    - [[#practical-examples][Practical Examples]]
    - [[#integration-with-claude-org-mode][Integration with claude-org-mode]]
...
This project references Anthropic's official [[https://github.com/anthropics/claude-agent-sdk-python][Claude Agent SDK (Python)]] as a submodule:

#+BEGIN_SRC bash
reference/claude-agent-sdk-python/   # Official Python SDK
#+END_SRC

The Python SDK provides the canonical implementation of the Claude Agent protocol. Key features
implemented here follow the same design patterns:
- Session management and conversation threading
- Tool use and permission control
- Streaming protocol and JSON message format
- Hook system for tool interception

When developing new features for =claude-agent.org=, consult the Python reference at
=reference/claude-agent-sdk-python/src/claude_agent_sdk/= for implementation guidance.
...
**** Transient Menu (Primary Interface)

The =claude-org-menu= command (=C-c C-/=) is the primary way to interact with claude-org.
It provides a visual command palette for all operations:
...
│ c Execute      i New block   n Info    │
│ k Cancel       s Story       l List    │
│ v Verbose      d SDD         p Perms   │
└────────────────────────────────────────┘
#+END_EXAMPLE

| Key | Action |
|-----+--------|
| =c= | Execute AI block (same as =C-c C-c=) |
| =k= | Cancel active query |
| =v= | Show verbose output buffer |
| =i= | Insert new AI block (with tag selection) |
| =s= | Insert story section (=:claude_chat:=) |
| =d= | Insert SDD structure |
| =j= | Jump to recent AI block |
| =n= | Show session info |
| =l= | List all sessions |
| =p= | Switch permission mode |

**** Jump to Recent AI Blocks

Quickly navigate to recently executed AI blocks with =C-c C-/ j= (or =M-x claude-org-jump-to-ai-block=).

This opens a completion list showing:
- Block status: =[✓]= completed, =[⋯]= in progress, =[✗]= error
- Hierarchical title: =Query Title > Parent Section > Grandparent=
- Relative time: =just now=, =5m ago=, =2h ago=, =3d ago=

#+BEGIN_EXAMPLE
[✓] Python JSON Parsing > Backend API > My Feature (5m ago)
[✓] Database Schema Design > Design Phase > My Feature (1h ago)
[⋯] Authentication Flow > Security Review > My Feature (just now)
#+END_EXAMPLE

*Features:*
- History persists across Emacs restarts (stored in =~/.emacs.d/claude-org/=)
- Navigation uses stable CUSTOM_ID links (survives file edits and heading renames)
- Auto-unfolds target section after navigation

**** Direct Keybindings

| Key | Command | Description |
|-----+---------+-------------|
| =C-c C-/= | =claude-org-menu= | Open transient command menu |
| =C-c C-c= | Execute query | Send AI block to Claude (org-babel) |
| =C-c C-k= | Cancel | Cancel active query |

**** Transient Setup

Transient is built into Emacs 28+. For earlier versions:
...
=acceptEdits= mode, permission functions are called for each tool request, allowing you to
allow, deny, or prompt for confirmation.
...
#+BEGIN_SRC elisp
(defun my-block-dangerous-bash (tool-name tool-input _context)
  "Block rm, sudo, and other dangerous bash commands."
  (when (equal tool-name "Bash")
    (let ((command (plist-get tool-input :command)))
      (when (string-match-p "\\(rm\\s+-rf\\|sudo\\|chmod 777\\)" command)
        (list :behavior "deny"
              :message "🚫 Dangerous command blocked for safety")))))

(add-hook 'claude-agent-permission-functions #'my-block-dangerous-bash)
#+END_SRC

**** Log All Tool Use

#+BEGIN_SRC elisp
(defun my-log-tool-use (tool-name tool-input _context)
  "Log every tool Claude uses."
  (message "[Claude Tool] %s at %s" tool-name (format-time-string "%H:%M:%S"))
  nil)  ; Return nil = let next function decide

(add-hook 'claude-agent-permission-functions #'my-log-tool-use)
#+END_SRC

**** Auto-approve Everything

#+BEGIN_SRC elisp
;; Replace default with auto-allow
(setq claude-agent-permission-functions
...
Claude will automatically use =evalElisp= to fetch the "Important Config" content,
then use that context to answer your question.

** SDD (Spec-Driven Development)

SDD is a structured workflow for feature development following the Atomic pattern:
...
When inserting new AI blocks in the Workflow section, use =C-c C-/= then =i= to select phase tags:
...
| Section | Tag | Purpose |
|---------+-----+---------|
| *Workflow* | =:sdd:= | AI interactions - all phases happen here |
...
| *Features* | =:features:= | Atomic TODO items with ORDERED tracking |
...
Control Claude's behavior through org tags and header arguments. This provides a declarative way to
activate different "modes" for your AI queries.

*** Phase Tags

Use org section tags to set the current development phase:
...
Combine with phase tags to adjust behavior:
...
,* Critical fix :code:strict:
,#+begin_src ai
Fix the memory leak - must pass all tests.
,#+end_src
#+END_EXAMPLE
...
Override or supplement behavior via block header arguments:

#+BEGIN_EXAMPLE
,* Task
,#+begin_src ai :phase code :tests :coverage 80
Implement the user preference API.
,#+end_src
#+END_EXAMPLE
...
| =:context= | TEXT | Add additional context |
| =:agent= | TYPE | Delegate to specialized subagent |

*** Combining Tags and Headers

Tags and headers combine naturally. Tags apply to all blocks in a section, headers can override per-block:

#+BEGIN_EXAMPLE
,* Backend API :code:security:
All blocks here get CODE + SECURITY behavior.
...
Add your own tags and headers:

#+BEGIN_SRC elisp
;; Custom tag for API development
(add-to-list 'claude-org-tag-behaviors
             '(api . "## API MODE
Follow REST conventions. Use proper HTTP methods.
Return appropriate status codes."))

;; Custom header for timeout
(add-to-list 'claude-org-header-behaviors
             '(:timeout . "Complete within %s minutes."))
#+END_SRC

Or use =M-x customize-variable= to edit =claude-org-tag-behaviors= and =claude-org-header-behaviors=.

*** File-Level Tag Definitions

Define available tags in your org file for quick access with =C-c C-c= on headings:
...
Claude-org includes several features to improve output readability:

*** Header Normalization

When Claude outputs org headers, they're automatically adjusted to fit your document hierarchy.
For example, if you execute a query inside a level-3 section, Claude's =* Header= becomes =**** Header=.
...
This helps when Claude generates very long paragraphs without natural breaks.
...
DOS (=\r\n=) and old Mac (=\r=) line endings are automatically converted to Unix (=\n=).

*** Auto Title Generation

When executing AI blocks under generic "Instruction N" headings, titles are automatically
generated using the Haiku model (fast and low-cost). The generated title replaces the
generic heading while preserving tags.

#+BEGIN_SRC elisp
;; Enable auto-title generation (default: t)
(setq claude-org-auto-generate-title t)

;; Only replace headings matching this pattern (default: "^Instruction [0-9]+$")
(setq claude-org-auto-title-heading-pattern "^Instruction [0-9]+$")

;; Disable auto-title generation
(setq claude-org-auto-generate-title nil)
#+END_SRC

Example transformation:
- Before: =*** Instruction 1 :claude_chat:=
- After: =*** Python JSON Parsing :claude_chat:=

Custom headings (not matching the pattern) are preserved unchanged.
...
Each analysis opens a new =*Claude Error Analysis N*= buffer with =claude-agent-chat-mode=, allowing follow-up questions.

*** Automatic Analysis (Optional)

To automatically analyze errors when the debugger appears, add to your config:

#+BEGIN_SRC elisp
;; Optional: auto-analyze on debugger entry
(add-hook 'debugger-mode-hook #'claude-agent-analyze-backtrace)
#+END_SRC

*** MCP Integration

For deeper analysis, Claude can query your live Emacs session using the =evalElisp= MCP tool:

#+BEGIN_SRC elisp
;; Configure MCP for error analysis
(setq claude-agent-error-mcp-config "~/.config/claude/mcp-emacs.json")
#+END_SRC

This allows Claude to inspect variable values, function definitions, and other context.

*** Configuration

#+BEGIN_SRC elisp
;; Debounce delay for rapid errors (default: 0.5 seconds)
(setq claude-agent-error-debounce-delay 0.5)

;; Custom system prompt file
(setq claude-agent-error-prompt-file "~/my-error-prompt.org")

;; MCP config for Emacs introspection
(setq claude-agent-error-mcp-config "~/.config/claude/mcp-emacs.json")
#+END_SRC
...
;; Error Analysis
(setq claude-agent-error-debounce-delay 0.5)  ; Debounce rapid errors
(setq claude-agent-error-prompt-file "~/my-prompt.org")  ; Custom prompt
(setq claude-agent-error-mcp-config "~/.config/claude/mcp.json")  ; MCP tools
#+END_SRC
...
;; Maximum line width hint for Claude (sent in system prompt)
(setq claude-org-line-width 170)

;; Auto-break long lines after sentence punctuation (-1 to disable)
;; When positive, lines exceeding this length are broken after . ! ?
(setq claude-org-auto-break-line-length 110)

;; Normalize org headers in output (adjust levels to fit document hierarchy)
(setq claude-org-normalize-headers t)

;; Auto-generate titles for "Instruction N" headings using Haiku
(setq claude-org-auto-generate-title t)
(setq claude-org-auto-title-heading-pattern "^Instruction [0-9]+$")
...
;; Permission mode for tools (default: acceptEdits)
