...
Emacs MCP org_* tools instead (which preserve org structure).
...
links like =[[file:notes.org::*Project Setup][Setup]]= in AI blocks, Claude uses MCP tools to
retrieve the linked content and inform its response.

*** Supported Link Types

| Link Format | MCP Tool Used |
|-------------+---------------|
| =[[*Heading]]= | =org_read_section= (current notebook) |
| =[[file:path.org::*Heading][desc]]= | =org_read_section= |
...
Claude will automatically call =org_read_section= to fetch the "Important Config" content,
...
4. *Update as you go*: Claude updates Research Output, Spec, Features via MCP org tools
