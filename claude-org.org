...
(defun claude-org--build-block-title-from-position (line-number)
  "Build a meaningful title by going to LINE-NUMBER and walking up headings.
Returns \"Story Name / Instruction Title\" format when possible.
Skips 'Workflow' headings as they're SDD containers.
Uses the deepest meaningful heading as instruction, shallowest as story."
      ...
      (save-restriction
        ...
        (condition-case nil
            (progn
              ...
              ;; Collect all meaningful headings (skip "Workflow")
              (let ((headings nil))
                (while (org-up-heading-safe)
                  (let ((heading (substring-no-properties (org-get-heading t t t t)))
                        (level (org-current-level)))
                    (unless (string= heading "Workflow")
                      (push (cons level heading) headings))))
                ;; headings is now ((level1 . "heading1") (level2 . "heading2") ...)
                ;; sorted from shallowest to deepest
                (cond
                 ;; No meaningful headings found
                 ((null headings) nil)
                 ;; Only one heading - just use it
                 ((= (length headings) 1)
                  (cdar headings))
                 ;; Multiple headings - use shallowest as story, deepest as instruction
                 (t
                  (let ((story (cdar headings))           ;; first = shallowest
                        (instruction (cdar (last headings)))) ;; last = deepest
                    (if (string= story instruction)
                        instruction
                      (format "%s / %s" story instruction)))))))
          (error nil))))))

(defun claude-org--format-block-candidate (entry)
  "Format ENTRY for display in completing-read.
ENTRY is (BLOCK-ID . PLIST).
Builds title dynamically from line number to always show current heading names."
  (let* ((plist (cdr entry))
         (status (plist-get plist :status))
         (line (plist-get plist :line))
         ;; Build title dynamically from line position, fall back to stored title for legacy
         (title (or (claude-org--build-block-title-from-position line)
