...
Includes :custom-id for stable navigation - titles are built dynamically when listing."
  (let ((entries (mapcar
                  (lambda (entry)
                    (let ((id (car entry))
                          (plist (cdr entry)))
                      ;; Remove :marker (not persistent), keep :custom-id for navigation
                      ;; Note: :title no longer stored - built dynamically from CUSTOM_ID
...
Only stores CUSTOM_ID for navigation - titles are built dynamically when listing."
  (claude-org--ensure-history-loaded)
  (let* ((block-id (format "block-%d-%s"
                           (line-number-at-pos (marker-position marker))
                           (format-time-string "%H%M%S")))
         ;; Only get CUSTOM_ID from parent heading - title built dynamically later
...
(defun claude-org--build-block-title-from-custom-id (custom-id)
  "Build a meaningful title by navigating to CUSTOM_ID and walking up headings.
Returns \"Story Name / Instruction Title\" format, or just instruction title
if no SDD story context is found."
  (when custom-id
    (save-excursion
      (save-restriction
        (widen)
        (condition-case nil
            (progn
              ...
              ;; Now at the heading with CUSTOM_ID, get its title
              (let* ((instruction-title (org-get-heading t t t t))
                     (story-name nil))
                ;; Walk up to find SDD story (look for :sdd: tag or story-like parent)
                (save-excursion
                  (while (and (not story-name) (org-up-heading-safe))
                    (let ((tags (org-get-tags nil t)))
                      ;; Check for SDD-related tags or feature-like headings
                      (when (or (member "sdd" tags)
                                (member "story" tags)
                                (member "feature" tags)
                                ;; Also check if this looks like a story heading (level 1-2)
                                (<= (org-current-level) 2))
                        (setq story-name (org-get-heading t t t t))))))
                ;; Build the combined title
                (if (and story-name
                         (not (string= story-name instruction-title)))
                    (format "%s / %s" story-name instruction-title)
                  (or instruction-title "Untitled"))))
          (error nil))))))

(defun claude-org--format-block-candidate (entry)
  "Format ENTRY for display in completing-read.
ENTRY is (BLOCK-ID . PLIST).
Builds title dynamically from CUSTOM_ID to always show current heading names."
  (let* ((plist (cdr entry))
         (status (plist-get plist :status))
         (custom-id (plist-get plist :custom-id))
         ;; Build title dynamically from CUSTOM_ID, fall back to stored title for legacy
         (title (or (claude-org--build-block-title-from-custom-id custom-id)
      ...
      (let* ((selection (completing-read "Jump to: " candidates nil t))
        ...
        (cond
         ...
         ((and custom-id (claude-org--goto-block-by-custom-id custom-id))
          ;; Successfully navigated via CUSTOM_ID
          nil)
