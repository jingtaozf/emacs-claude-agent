...
Loads prompt from =docs/prompts/tags/TAG.org=.

#+BEGIN_SRC elisp
(defun claude-org--tag-prompt (tag)
  "Get behavior prompt for TAG symbol.
Loads from docs/prompts/tags/TAG.org at runtime."
...
1. Workflow :sdd: with Research/Design/Planning/Implementation phases
...
- Workflow: AI interaction happens here (parallel research → design → implementation)
- Research Output: Synthesized findings from parallel research agents
...
Aligns section level with previous SDD if any exists, otherwise uses top level."
  ...
  (let* ((name (read-string "Feature name: "))
    ...
    ;; 1. Insert Workflow section with phases
    (insert stars2 " Workflow :sdd:\n\n")
    (dolist (phase '(("Research" . "research")
                     ("Design" . "design")
                     ("Planning" . "planning")
                     ("Implementation" . "implementation")))
      (insert stars3 " " (car phase) " :" (cdr phase) ":\n\n"))

    ;; 2. Insert Research Output section for parallel agent findings
    ...
    ;; 4. Insert Features section with ORDERED property
    (insert stars2 " Features :features:\n")
    (insert ":PROPERTIES:\n")
    (insert ":ORDERED: t\n")
    (insert ":END:\n\n")

    ;; Position cursor at Research section (search backwards)
    (re-search-backward "^\\*+ Research :research:")
    (end-of-line)
    (insert "\n")
...
(transient-define-prefix claude-org-menu ()
  ...
  ["Claude Org"
   ...
   ["Insert"
    ("n" "New AI block" claude-org-insert-block)
    ("s" "Story section" claude-org-insert-story)
    ("d" "SDD structure" claude-org-insert-sdd)]
   ["Session"
    ("i" "Session info" claude-org-show-session-info)
