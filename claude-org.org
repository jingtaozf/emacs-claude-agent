...
(defun claude-org--discover-slash-commands ()
  "Discover all available slash commands from user and project .claude/commands.
Returns list of command strings with / prefix."
  (let ((commands (copy-sequence claude-org-builtin-slash-commands))
    ...
    ;; Remove duplicates (project overrides user)
  ...
  (cl-case command
    ...
    (prefix (when (claude-org--in-ai-block-p)
              (if (and (save-excursion
                  ...
                  ;; We match - return prefix and stop other backends
                  (cons (match-string 0) t)
...
(defun claude-org--send-request (prompt)
  ...
  (let* ((session-key (claude-org--current-session-key))
         ...
         ;; Build IDE context with project-root as cwd
         (ide-context (when claude-org-include-ide-context
                        (claude-agent-collect-ide-context (or project-root default-directory))))
         ...
         ;; Collect environment variables from org properties
         (env-vars (claude-org--get-env-vars))
         ;; Build options
