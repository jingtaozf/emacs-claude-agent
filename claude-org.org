...
** Org Link Resolution Prompt

When users include org links in AI blocks, Claude should resolve them autonomously
using MCP tools. This prompt instructs Claude on how to recognize and fetch context
from various org link types.

#+NAME: link-resolution-prompt
#+BEGIN_SRC elisp
(defconst claude-org-link-resolution-prompt
  "Org Link Resolution:
...
- Resolve multiple links in parallel when independent"
  "System prompt section explaining org link resolution to Claude.")

;; Add link resolution prompt to defaults
(add-to-list 'claude-org-default-system-prompts claude-org-link-resolution-prompt t)
#+END_SRC
