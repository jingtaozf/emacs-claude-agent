...
(defcustom claude-org-allowed-tools '("Read(**)" "Glob(**)" "Grep(**)")
...
(defcustom claude-org-mcp-server-port 9999
  "Port for the MCP server.
Used when auto-starting the MCP server."
  :type 'integer
  :group 'claude-org)
...
(defcustom claude-org-default-system-prompts
  '("You are communicating with a senior developer through Emacs org-mode.
...
- Markdown formatting renders well (headers, lists, code blocks)
- For code examples, use standard markdown code fences
...
(defconst claude-org--permission-mode-map
  ...
  "Map permission preset names to CLI permission-mode values.")
#+END_SRC
...
Returns nil if not set."
  (or
   ...
   (org-entry-get nil claude-org-sdk-uuid-property t)
...
Stores at the appropriate scope based on session configuration:
- If session ID is defined in a section, stores in section property drawer
- Otherwise stores at file level."
  (let ((scope (claude-org--get-session-scope)))
    (if (eq scope 'section)
        ;; Section-level: find the heading that defines the session and set there
        (save-excursion
...
Removes the property from the appropriate scope."
  (let ((scope (claude-org--get-session-scope)))
    (if (eq scope 'section)
...
Returns nil if not set."
  (or
   ...
   (save-excursion
     ...
     (when (re-search-forward
            (format "^#\\+PROPERTY:[ \t]+%s[ \t]+\\(.+\\)$"
                    claude-org-permission-property)
            nil t)
       (string-trim (match-string-no-properties 1))))))
...
Returns CLI permission-mode value."
  (let ((preset (or (claude-org--get-permission-mode-property)
                    ...
                    "readonly")))
    (or (cdr (assoc preset claude-org--permission-mode-map))
        "default")))
...
(defun claude-org--permission-mode-short ()
  ...
  (pcase (claude-org--get-permission-mode-display)
    ("readonly" "RO")
    ("accept-edits" "ED")
    ("plan" "PL")
    ("bypass" "BP")
  ...
  (let ((prop-value
         (or
          ...
          (save-excursion
            ...
            (when (re-search-forward
                   (format "^#\\+PROPERTY:[ \t]+%s[ \t]+\\(.+\\)$"
                           claude-org-setting-sources-property)
                   nil t)
              (string-trim (match-string-no-properties 1)))))))
    (if prop-value
...
Sets at section level if at a heading, otherwise file level."
  (if (org-at-heading-p)
      ;; Section-level: set property in drawer
...
Starts the MCP server on `claude-org-mcp-server-port' if not already running."
  (when (and claude-org-auto-start-mcp-server
             (fboundp 'emacs-mcp-server-running-p)
             (not (emacs-mcp-server-running-p)))
    (emacs-mcp-server-start claude-org-mcp-server-port)))
...
Attempts session recovery if error indicates expired session."
  (let ((error-msg (claude-org--extract-error-message error)))
    (if (and (claude-agent--session-expired-p error-msg)
        ...
        ;; Session expired - clear invalid UUID and recover
        (progn
