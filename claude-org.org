...
- Use org-mode syntax, NOT markdown:
  - Headers: * for h1, ** for h2, *** for h3 (not # or ##)
  - Bold: *bold* (not **bold**)
...
- Line width: Keep lines under 170 characters, break long lines naturally
...
(defun claude-org--get-current-section-level ()
  "Get the org section level containing point.
Returns the level (1 for *, 2 for **, etc.) or 0 if not in a section."
  (save-excursion
    (if (org-before-first-heading-p)
        0
      (org-back-to-heading t)
...
(defun claude-org--send-request (prompt)
  ...
  (let* ((session-key (claude-org--current-session-key))
         ...
         ;; Get current org section level for output hint
         (current-level (claude-org--get-current-section-level))
         (output-level (if (> current-level 0) (1+ current-level) 0))
         ;; Build IDE context
