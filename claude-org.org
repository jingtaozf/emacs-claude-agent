...
#+BEGIN_SRC elisp
(defun claude-org--normalize-headers-in-text (text target-level pending-line)
  "Normalize org headers in TEXT to start at TARGET-LEVEL.
PENDING-LINE is incomplete text from previous token (no trailing newline).
Returns (normalized-text . new-pending-line).
...
Only buffers incomplete lines that could potentially be headers (start with *)."
  (if (not claude-org-normalize-headers)
      ;; Normalization disabled - just handle pending line
      (cons (concat pending-line text) "")
    ;; Combine pending line with new text
    (let* ((combined (concat pending-line text))
           (has-trailing-newline (string-suffix-p "\n" combined))
           (lines (split-string combined "\n"))
           (last-idx (1- (length lines)))
           (result-lines '())
           (new-pending "")
           (offset (1- target-level)))  ; offset to add to Claude's levels
      ;; Process lines
      (cl-loop for line in lines
               for idx from 0
               do (cond
                   ;; Last line without trailing newline
                   ((and (= idx last-idx) (not has-trailing-newline))
                    ;; Only buffer if it could be a header (starts with *)
                    ;; Otherwise output immediately to avoid buffering all text
                    (if (string-prefix-p "*" line)
                        (setq new-pending line)
                      ;; Not a potential header - output immediately (no newline)
                      (push line result-lines)))
                   ;; Empty string after trailing newline - skip
                   ((and (= idx last-idx) has-trailing-newline (string= line ""))
                    nil)
                   ;; Normal complete line - process and add newline
                   (t
                    (push (concat (claude-org--normalize-header-line line offset) "\n")
                          result-lines))))
      (cons (apply #'concat (nreverse result-lines))
            new-pending))))

(defun claude-org--normalize-header-line (line offset)
  "Normalize a single LINE by adding OFFSET to any org header level.
Returns the normalized line."
  ;; Match org header: starts with one or more *, then space, then content
  (if (string-match "^\\(\\*+\\) " line)
      (let* ((stars (match-string 1 line))
             (rest (substring line (match-end 1)))
             (new-level (+ (length stars) offset))
             (new-stars (make-string (max 1 new-level) ?*)))
        (concat new-stars rest))
    ;; Not a header, return unchanged
    line))

(defun claude-org--flush-pending-line (session-key)
  "Flush any pending line for SESSION-KEY.
Called when response is complete to output remaining text."
  (let ((pending (claude-org--session-get session-key :pending-line)))
      ...
      (let ((marker (claude-org--session-get session-key :marker)))
        (when (and marker (marker-buffer marker))
          (with-current-buffer (marker-buffer marker)
            (save-excursion
              (goto-char marker)
              ;; Normalize the pending line too
              (let* ((target-level (or (claude-org--session-get session-key :section-level) 1))
                     (offset (1- target-level))
                     (normalized (claude-org--normalize-header-line pending offset)))
                (insert normalized))
              (set-marker marker (point))))))
      (claude-org--session-put session-key :pending-line ""))))
#+END_SRC
    ...
    (when (and marker (marker-buffer marker))
      (with-current-buffer (marker-buffer marker)
        (let* ((target-level (1+ (or (claude-org--session-get session-key :section-level) 0)))
               (pending (or (claude-org--session-get session-key :pending-line) ""))
               (result (claude-org--normalize-headers-in-text text target-level pending))
               (normalized-text (car result))
               (new-pending (cdr result)))
          ;; Save new pending line
          (claude-org--session-put session-key :pending-line new-pending)
...
(defun claude-org--handle-complete (session-key result)
  ...
  ;; Flush any pending line from header normalization
  (claude-org--flush-pending-line session-key)
...
(ert-deftest claude-org-test-normalize-header-line ()
...
(ert-deftest claude-org-test-normalize-headers-token-boundary ()
