...
Default system prompts establish Claude's persona and org-mode awareness.
These are prepended to every query. Add domain-specific prompts via
=add-to-list= rather than replacing these defaults.
...
Org tags (like =:explore:=, =:review:=) trigger specific behavior prompts.
This enables workflow phases: explore → plan → code → test → review → commit.

Phase tags define development workflow stages. Modifier tags (=strict=,
=security=, =no_edit=, =parallel=) adjust behavior within any phase.

#+NAME: custom-tag-behaviors
#+BEGIN_SRC elisp
(defcustom claude-org-tag-behaviors
...
Use Task agents concurrently for independent subtasks."))
  ...
  "Alist mapping org tags to behavior prompt text.
Each entry: (TAG-SYMBOL . PROMPT-STRING)

Tags activate via org section tags like :explore: or :review:.
Multiple tags combine (alphabetically for consistency).

To add custom tags:
  (add-to-list \\='claude-org-tag-behaviors
               \\='(my-tag . \"My custom behavior prompt\"))"
  :type '(alist :key-type symbol :value-type string)
  :group 'claude-org)
#+END_SRC

** Header Argument Behaviors

Header arguments (=#+begin_src ai :phase plan :strict t=) provide inline
behavior control. Unlike tags which apply to whole sections, header args
apply only to that specific block.

Values can be:
- Template strings with =%s= for value substitution
- Fixed strings for boolean flags
- Alists for enum-style mappings (like =:phase=)

#+NAME: custom-header-behaviors
#+BEGIN_SRC elisp
(defcustom claude-org-header-behaviors
  '((:phase . (("explore" . "## EXPLORE PHASE\nFocus on understanding. Do NOT modify files.")
...
Delegate to specialized %s agent via Task tool."))
  ...
  "Alist mapping header arguments to prompt templates.
Each entry: (HEADER-KEYWORD . VALUE) where VALUE is:
  - STRING with %s: Template, value substituted at %s
  - STRING without %s: Fixed prompt (for boolean headers like :tests t)
  - ALIST: Map specific values to prompts (for enum headers like :phase)

Headers activate via #+begin_src ai :header value syntax.
...
(defun claude-org--build-system-prompt ()
  "Build complete system prompt from defaults, org sections, and behaviors.
Combines:
1. `claude-org-default-system-prompts' (base prompts)
2. :system_prompt: tagged org sections
...
4. Line width instruction based on `claude-org-line-width'"
  (let ((defaults (when claude-org-default-system-prompts
    ...
    ;; Combine all parts: defaults + org-prompts + behaviors + line-width
    (let ((parts (delq nil (list defaults org-prompts behavior-prompt line-width-hint))))
      (when parts
        (string-join parts "\n\n")))))
#+END_SRC
...
(defun claude-org--tag-prompt (tag)
  ...
  (alist-get tag claude-org-tag-behaviors))
#+END_SRC
...
Returns nil if header not found or value is nil/empty."
  (when value
    (when-let ((entry (alist-get header claude-org-header-behaviors)))
      (cond
       ;; Alist: lookup value directly (for enum headers like :phase)
       ((and (consp entry) (consp (car entry)))
        (alist-get value entry nil nil #'equal))
       ;; Template with %s: format with value
       ((and (stringp entry) (string-match-p "%s" entry))
        (format entry value))
       ;; Fixed string: return if value is truthy (for boolean headers)
       ((stringp entry)
        entry)))))
#+END_SRC
...
Returns combined prompt string or nil if no behaviors active."
  (let* ((config (claude-org--collect-behavior-config))
         ...
         (parts '()))
    ;; Collect tag prompts (sorted alphabetically for consistency)
    (dolist (tag-str (sort (copy-sequence tags) #'string<))
      (when-let ((prompt (claude-org--tag-prompt (intern tag-str))))
        (push prompt parts)))
...
** Why This Hook Cannot Intercept Edit/Write Tools

The =claude-agent-pre-tool-use-hook= is triggered when Claude Code CLI sends a
=can_use_tool= control request to Emacs. However, Claude Code's built-in Edit/Write
tools are *not* routed through this mechanism:

| Tool Type | Execution Path | Hook Intercept? |
|-----------+----------------+-----------------|
| Claude Code's Edit/Write | Claude CLI → filesystem directly | ❌ No |
| MCP tools (evalElisp, org_*) | Claude CLI → HTTP → Emacs MCP server | ❌ No (MCP protocol) |
| Emacs-delegated tools | Claude CLI → control_request → Emacs | ✅ Yes |

In =bypassPermissions= mode (which we use for efficiency), the CLI auto-approves
Edit/Write without asking Emacs. Even in other modes, the CLI handles these tools
internally rather than delegating to Emacs.

*Current Solution:* System prompt instructions tell Claude to use MCP org_* tools
for .org files. This relies on prompt compliance rather than enforcement.

** Hook Implementation (Non-Functional)

This code remains as reference for the intended behavior. If Claude Code CLI
ever adds a hook mechanism for built-in tools, this pattern could be adapted.

#+NAME: org-protect-hook
#+BEGIN_SRC elisp
...
** Hook Installation (Non-Functional)

These functions install/remove the hook, but since the hook never triggers for
Edit/Write tools, they have no practical effect. Kept for potential future use.

#+NAME: org-protect-hook-install
#+BEGIN_SRC elisp
...
NOTE: This hook does not actually intercept Edit/Write tools.
See documentation in 'Org File Protection Hook' section."
