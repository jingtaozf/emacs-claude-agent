...
Includes :custom-id for stable navigation across sessions."
  (let ((entries (mapcar
                  (lambda (entry)
                    (let ((id (car entry))
                          (plist (cdr entry)))
                      ;; Remove :marker (not persistent), keep :custom-id
...
MARKER points to the AI block."
  ...
  (let* ((block-id (format "block-%d-%s"
         ...
         ;; Get title and CUSTOM_ID from parent heading
         (heading-info (save-excursion
                         (goto-char (marker-position marker))
                         (when (org-up-heading-safe)
                           (list :title (org-get-heading t t t t)
                                 :custom-id (org-entry-get nil "CUSTOM_ID")))))
         (title (plist-get heading-info :title))
         (custom-id (plist-get heading-info :custom-id))
...
ENTRY is (BLOCK-ID . PLIST)."
  (let* ((plist (cdr entry))
    ...
    (format "[%s] %s (%s)"
            status-icon
            (truncate-string-to-width (or title "Untitled") 60 nil nil "…")
            time-str)))
